    ...
    <div className={`${arvoFont.variable}`}>
      {/* SECTION 1 */}
        <section id='section1' className='h-screen bg-cover  bg-center bg-no-repeat' style={{ backgroundImage: "url('/image/bg-sec1.avif')" }}>
            <div className='bg-white h-[70%] md:h-[60%] text-3xl md:px-[3rem] '>
              <h1 className="text-base md:pt-[8rem] font-semibold text-center font-arvo">Welcome to</h1>
              <h1 className='flex items-center justify-center pt-[1rem] md:pt-[1rem] font-arvo head_text'>{text}</h1>
              <p className='text-center text-sm md:text-[20px] md:pt-[1rem] font-arvo'>We are a drinking club with a running problem.</p>
              <p className='text-center text-sm font-arvo md:text-xl'>We drink beer to save water</p>
              <p className='text-center md:pt-[12px] text-sm font-arvo md:text-xl flex items-center justify-center gap-2'>Join us every Friday at 5pm!
